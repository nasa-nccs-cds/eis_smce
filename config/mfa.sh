    ...
    then
      ...
      then
        ...
        echo "Session token found.  This can not be used to generate a new token.
          ...
          set the variables to the normal keys." >&2
        ...
        then
          ...
          echo "Can not identify who you are.  Looking for a line like arn:aws:iam::.....:user/FOO_BAR
          but did not find one in the output of aws sts get-caller-identity $identity" >&2
          ...
          then
            ...
            aws iam list-mfa-devices --username \"$username\" $mfa" >&2
