...
dsets = [
    dict(   input = f"file://{input_dir}/ROUTING/{month}/LIS_RST_HYMAP2_router" + "_{sample}.d01.nc",
            output = f"s3://{bucket}/{s3_prefix}/ROUTING/IS_RST_HYMAP2_router.d01.zarr" ),

    dict(   input=f"file://{input_dir}/ROUTING/{month}/LIS_HIST" + "_{sample}.d01.nc",
            output=f"s3://{bucket}/{s3_prefix}/ROUTING/LIS_HIST.d01.zarr" ),

    dict(   input=f"file://{input_dir}/SURFACEMODEL/{month}/LIS_HIST" + "_{sample}.d01.nc",
            output=f"s3://{bucket}/{s3_prefix}/SURFACEMODEL/LIS_HIST.d01.zarr" ),

    dict(   input=f"file://{input_dir}/SURFACEMODEL/{month}/LIS_RST_NOAHMP401" + "_{sample}.d01.nc",
...
if __name__ == '__main__':
    ...
    for dset in dsets:
        ...
        zs = h4s.export( output, bucket=bucket, merge_dim = "sample", **dset )
        cm().addEntry( zs  )
